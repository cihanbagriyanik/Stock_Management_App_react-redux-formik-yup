...
export default function PurchasesModal({ open, handleClose, info, setInfo }) {
  ...
  const { createPurchases, updatePurchases } = useStockCall();
  const { firms, products, brands } = useSelector((state) => state.purchases);
  ...
  const handleSubmit = (e) => {
    ...
    if (info._id) {
      updatePurchases("purchases", info);
    } else {
      createPurchases("purchases", info);
    }
